...
generate_url <- function(filename) {
  return(paste0("https://raw.githubusercontent.com/jessedebolt/MSDS-Capstone/main/data/", filename))
}
...
### Iterate through each CSV file, read it into R, and store it as an individual data frame
```{r}
# cdc <- list()
...
Create new DF with non-num columns excluded, change NA to NULL ("") to see if that fixes infinite issue

all(sapply(cdc, is.finite))
...
```{r results='hide'}
pr_income <- prcomp(x = select(cdc, -c('Prev-Coronary_Heart_Disease', 'SEED-UR-Urban-Rural', 'cnty_fips', 'display_name')), scale = T, center = T)
