...
for (df_name in df_names) {
  ...
  if(all(c("cnty_fips", "display_name") %in% colnames(df))){
    ...
    if (nrow(final_df) == 0){
      ...
      names(final_df) <- gsub("\\_Value$", "", names(final_df))
    }
  }
}

# Remove extra population variable
#final_df <- final_df %>% select(-c(`final_df_Demo-Total_Population`))
...
final_df$`SEED-UR-Urban-Rural` <- ifelse(final_df$`SEED-UR-Urban-Rural`
                                         == 1, "Large_Urban",
                                  ifelse(final_df$`SEED-UR-Urban-Rural`
                                         == 2,"LargeFringe_Urban",
                                  ifelse(final_df$`SEED-UR-Urban-Rural` 
                                         == 3,"MediumSmall_Urban", 
                                  ifelse(final_df$`SEED-UR-Urban-Rural`
                                         == 4,"Rural",
                                  final_df$`SEED-UR-Urban-Rural`))))

### Replacing '-1' with 'NA' under assumption that these are truly missing elements
final_df[final_df == -1] <- NA
...
write_csv(final_df, "CDC_all.csv")  
