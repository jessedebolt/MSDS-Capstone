...
cdc <- cdc %>% 
  mutate(
    PCP = coalesce(PCP, pcp),
    CardioPhys = coalesce(CardioPhys, cardio)
  )

# Remove the temporary columns
cdc <- cdc %>% select(-pcp, -cardio)
...
cdc$Hosp100k <- (cdc$Hospitals / cdc$population) * 100000
...
```{r}
# Create column for number of hospitals per 100k residents
cdc$Pharm100k <- (cdc$Pharmacies / cdc$population) * 100000
...
cdc$TotalPCP <- cdc$population / cdc$PrimaryCarePhys
# Create columns for number of physicians per 1k residents
cdc$PSP1k <- (cdc$TotalPCP / cdc$population) * 1000
...
```{r}
# Calculate the number of physicians
cdc$TotalCardio <- cdc$population / cdc$CardioPhys
# Create columns for number of physicians per 1k residents
cdc$Cardio100k <- (cdc$CardioPhys / cdc$population) * 100000
...
Services-CIC = Cardiac Intensive Care
Services-CR = Cardiovascular Rehabilitation
>>>>>>> f5d595b6f603f4ebf1704dfb08851198a18ab311
UR = Urban/Rural