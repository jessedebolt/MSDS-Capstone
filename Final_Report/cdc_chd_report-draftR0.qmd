...
# Train test split
cdc_index_top5 <- createDataPartition(cdc_clean$Stroke, p = 0.8, list = FALSE)
cdc_tr_top5 <- cdc_clean[cdc_index_top5, ]
cdc_te_top5 <- cdc_clean[-cdc_index_top5, ]
...
# Fit the model
model_top5 <- train(Stroke ~ UrbanRural + Age65Plus + Poverty + Smoker + bpmUse + SNAPrecipients + Age65Plus*UrbanRural + Poverty*UrbanRural + Smoker*UrbanRural + bpmUse*UrbanRural + SNAPrecipients*UrbanRural,  
                    data=cdc_tr_top5, 
                    method="lm", 
                    trControl = control_top5)

summ(model_top5$finalModel)

```

Let’s examine a couple of these coefficients.  When looking at the percentage of people who smoke, we get the coefficient of Smoker 0.054767.  Which means for every extra 1% of people who smoke, we'd expect about a 0.05% increase in stroke prevalence, not considering the other factors.  Now, if we consider adding an interaction with urban or rural status, we get a coefficient of `UrbanRural1:Smoker` 0.010469. This suggests that city places with more smokers see about a 0.01% higher stroke prevalence than we'd expect just from being a city and having more smokers separately.  

Another one we can look at, is the coefficient for people who are prescribed blood pressure medication, we get `bpmUse` 0.057698. For every extra 1% of people who use blood pressure medication, we'd expect about a 0.057% increase in stroke prevalence, not considering the other factors.  Then if we add the interaction to consider urban or rural status, we get `UrbanRural1:bpmUse` -0.033723. This suggests that city places where more people use blood pressure medication see about a 0.034% lower stroke prevalence than we'd expect just from being a city and the blood pressure medication usage separately.  
...
# Train test split
cdc_index_top5 <- createDataPartition(cdc_clean$HighBP, p = 0.8, list = FALSE)
cdc_tr_top5 <- cdc_clean[cdc_index_top5, ]
cdc_te_top5 <- cdc_clean[-cdc_index_top5, ]
...
# Fit the model
model_top5 <- train(HighBP ~ UrbanRural + Age65Plus + Poverty + Smoker + bpmUse + SNAPrecipients + Age65Plus*UrbanRural + Poverty*UrbanRural + Smoker*UrbanRural + bpmUse*UrbanRural + SNAPrecipients*UrbanRural,  
                    data=cdc_tr_top5, 
                    method="lm", 
                    trControl = control_top5)
...
Looking closer at the percentage of people who have less. Than a college education, we see the coefficient of `EdLessColl` 0.209690. Which means, for every extra 1% of people who didn't go to college, we'd expect about a 0.21% increase in high blood pressure, not considering the other factors. This could be due to various things like diet, lifestyle, and stress levels. Then if we add the interaction with urban or rural, we get the coefficient of `UrbanRural1:EdLessColl` -0.038142. Here, city places with more people who didn't go to college see about a 0.04% lower high blood pressure rate than we'd expect just from being a city and having fewer college graduates separately.  

If we examine cholesterol screening, we get the coefficient CholScreen 0.569438. So, for every extra 1% of people who get their cholesterol checked, we'd expect about a 0.57% increase in high blood pressure, not considering the other factors. This could be because the more people get checked, the more likely we are to find high blood pressure. However, then we we add the interaction for urban or rural, we see that we get a coefficient of `UrbanRural1:CholScreen` -0.283930. This suggests that city places with more people getting their cholesterol checked see about a 0.28% lower high blood pressure rate than we'd expect just from being a city and the cholesterol checks separately.  
...
# Train test split
cdc_index_top5 <- createDataPartition(cdc_clean$HighChol, p = 0.8, list = FALSE)
cdc_tr_top5 <- cdc_clean[cdc_index_top5, ]
cdc_te_top5 <- cdc_clean[-cdc_index_top5, ]
...
# Fit the model
model_top5 <- train(HighChol ~ UrbanRural + Age65Plus + Poverty + Smoker + bpmUse + SNAPrecipients + Age65Plus*UrbanRural + Poverty*UrbanRural + Smoker*UrbanRural + bpmUse*UrbanRural + SNAPrecipients*UrbanRural,  
                    data=cdc_tr_top5, 
                    method="lm", 
                    trControl = control_top5)
...
## **RECOMMENDATIONS**
The main indicators of coronary heart disease are high blood pressure, high cholesterol, and stroke. These factors are typical aspects of CHD, hence their significance. Upon examination of the top five indicators for each of these, we note some overlap among ten different factors, as depicted in Figure 2 above.  
