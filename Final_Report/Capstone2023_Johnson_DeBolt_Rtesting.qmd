...
```{r}
library(ggplot2)
library(viridis) # for a colorblind-friendly and print-friendly color palette

# Create a diverging color palette
col_func <- colorRampPalette(c("blue", "white", "red"))

# Create map for Rural areas
p1 <- plot_usmap(data = cdc_rural, values = "CHD", color = "transparent", lwd = 0.05) +
  scale_fill_gradientn(colors = col_func(20), na.value = "lightgrey", name = "CHD",
                       label = scales::comma) +
  theme(legend.position = "right") +
  labs(title = "Coronary Heart Disease Prevalence by County (Rural)")

# Create map for Urban areas
p2 <- plot_usmap(data = cdc_urban, values = "CHD", color = "transparent", lwd = 0.1) +
  scale_fill_gradientn(colors = col_func(20), na.value = "lightgrey", name = "CHD",
                       label = scales::comma) +
  theme(legend.position = "right") +
  labs(title = "Coronary Heart Disease Prevalence by County (Urban)")
...
# Normalize CHD values to [0, 1] range
normalize <- function(x) {
    return((x - min(x, na.rm = TRUE)) / (max(x, na.rm = TRUE) - min(x, na.rm = TRUE)))
}

cdc$CHD_normalized <- normalize(cdc$CHD)

# Assign color based on UrbanRural status and alpha based on normalized CHD values
cdc$fill <- ifelse(grepl("Urban", cdc$UrbanRural),
                   scales::alpha("blue", cdc$CHD_normalized),
                   scales::alpha("green", cdc$CHD_normalized))

# Now we plot the combined map with colors based on the new fill column
plot_usmap(data = cdc, values = "fill", color = "transparent", lwd = 0.05) +
  scale_fill_identity() +
  theme(legend.position = "right") +
  labs(title = "Coronary Heart Disease Prevalence by County")

```
##save as above but saves it
```{r}
...
# Now we plot the combined map with colors based on the new fill column
map_blu_grn <- plot_usmap(data = cdc, values = "fill", color = "transparent", lwd = 0.05) +
  scale_fill_identity() +
  theme(legend.position = "right") +
  labs(title = "Coronary Heart Disease Prevalence by County")

ggsave(map_blu_grn, filename = "map_blu_grn.png", bg = "transparent")
```

```{r}
# Assign color based on UrbanRural status
cdc$fill <- ifelse(grepl("Urban", cdc$UrbanRural), "blue", "green")

# Apply alpha to the fill color
cdc$fill <- scales::alpha(cdc$fill, cdc$CHD_normalized)

# Now we plot the combined map with colors (and alpha) based on the fill column
map_blu_grn <- plot_usmap(data = cdc, values = "fill", color = "transparent", lwd = 0.05) +
  scale_fill_identity() +
  theme(legend.position = "right") +
  labs(title = "Coronary Heart Disease Prevalence by County")

ggsave(map_blu_grn, filename = "map_blu_grn2.png", bg = "transparent")
map_blu_grn
...
```{r}
# Square root transform CHD values, then normalize to [0, 1] range
cdc$CHD_transformed <- sqrt(cdc$CHD)
cdc$CHD_normalized <- normalize(cdc$CHD_transformed)

# Assign color based on UrbanRural status and alpha based on normalized CHD values
cdc$fill <- ifelse(grepl("Urban", cdc$UrbanRural),
                   scales::alpha("blue", cdc$CHD_normalized),
                   scales::alpha("#007700", cdc$CHD_normalized))
...
##All white
```{r}
# Assign white color to all counties
cdc$fill <- "white"

# Now we plot the combined map with colors based on the new fill column
map_white <- plot_usmap(data = cdc, values = "fill", color = "transparent", lwd = 0.05) +
  scale_fill_identity() +
  theme(legend.position = "none")

ggsave(map_white, filename = "map_white.png", bg = "transparent")
...
colors <- c("Large_Urban" = "blue", "LargeFringe_Urban" = "dodgerblue", "MediumSmall_Urban" = "cyan", "Rural" = "green")

four_bar <- ggplot(data = cdc_summary, aes(x = `UrbanRural`, y = `CHD`, fill = `UrbanRural`)) +
  geom_bar(stat = "identity", position = "dodge") +
  geom_hline(yintercept = avg_non_rural, linetype = "dashed", color = "red", size = 1) +
  geom_line(data = data.frame(UrbanRural = unique(cdc_summary$UrbanRural), CHD = avg_non_rural),
            aes(color = "Average Non-Rural"), linetype = "dashed") +
...
ggsave(four_bar, filename = "four_bar.png", width = 8, height = 6)
...
# Select top 5 and bottom 5 counties for urban and rural
highest_urban <- cdc_urban %>%
  arrange(desc(CHD)) %>%
  slice(1:5)

lowest_urban <- cdc_urban %>%
  arrange(CHD) %>%
  slice(1:5)

highest_rural <- cdc_rural %>%
  arrange(desc(CHD)) %>%
  slice(1:5)

lowest_rural <- cdc_rural %>%
  arrange(CHD) %>%
  slice(1:5)

# Combine the data frames
combined_prevalence <- rbind(
  mutate(highest_urban, group = "Highest", fill = "Urban"),  # Cornflower blue
  mutate(lowest_urban, group = "Lowest", fill = "Urban"),   # Cornflower blue
  mutate(highest_rural, group = "Highest", fill = "Rural"),
  mutate(lowest_rural, group = "Lowest", fill = "Rural")
)

# Create a new column for labels on y-axis: "County, State"
combined_prevalence$label <- paste(combined_prevalence$county, combined_prevalence$state, sep = ", ")

# Create the plot
hi_lo <- ggplot(combined_prevalence, aes(x = reorder(label, CHD), y = CHD, fill = fill)) +
  geom_bar(stat = "identity") +
  geom_text(aes(label = sprintf("%.1f%%", CHD)), hjust = 1.2, size = 3) +
  coord_flip() +
  facet_grid(group ~ ., scales = "free", space = "free") +
  scale_fill_manual(values = c("Urban" = "#6495ED", "Rural" = "green"), name = "Area") +
  labs(y = "CHD Prevalence") +
  theme_minimal() +
  theme(
    ...
    axis.ticks.x = element_blank(),
    strip.text = element_text(size = 13),
    plot.margin = margin(1, 1, 1, 1, "cm"),  # Increase right margin
    legend.position = "bottom"
  ) +
  ggtitle("Top and Bottom 5 Counties by CHD Prevalence (%)")

ggsave(hi_lo, filename = "hi_lo.png", width = 8, height = 5)
...
# Create a single plot with multiple panels
mountain <- ggplot(cdc_long, aes(x = CHD, fill = UrbanRural)) +
  geom_density(alpha = 0.5) +
  facet_wrap(~ Variable, scales = "free") +
  labs(x = "Coronary Heart Disease Prevalence", y = "Density") +
  theme_minimal() +
...
ggsave(mountain, filename = "mountain.png", width = 8, height = 5)
...
ggsave(s1, filename = "s1.png", width = 8, height = 5)
...
```{r}
# Select relevant columns
cdc2 <- cdc %>% select(Smoker, CHD, HighBP, HighChol, Stroke, UrbanRural)
...
# Create a single plot with multiple panels
s2 <- ggplot(cdc_long, aes(x = Smoker, y = Value, color = UrbanRural)) +
  geom_point(alpha = 0.1) +
  geom_smooth(method = "lm", se = FALSE, aes(group = UrbanRural)) +
  facet_wrap(~ Variable, scales = "free_y") +
  scale_color_manual(values = c("Rural" = "green", "Urban" = "blue")) +
  labs(x = "Smoker", y = "Value", color = "Urban/Rural") +
  theme_minimal()

# Print the plot
print(s1)

ggsave(s2, filename = "s2.png", width = 8, height = 5)
...
```{r echo=FALSE, fig.align='center', fig.cap='Figure 3: Variable Connections with Statistically Significant Interactions'}
