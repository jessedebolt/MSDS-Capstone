...
date: "`r Sys.Date()`"
format: html
subtitle: Tertiary document for specific (HighBP) target models
---
...
# Train test split
cdc_index_full <- createDataPartition(cdc$HighBP, p = 0.8, list = FALSE)
...
# Fit the model
model_full <- train(HighBP ~ . -county -state, 
                    data=cdc_tr_full, 
                    method="lm", 
...
# Train test split
cdc_index_main <- createDataPartition(cdc$HighBP, p = 0.8, list = FALSE)
...
# Fit the model
model_main <- train(HighBP ~ . -county -state -CHD -HighChol -Stroke , 
                    data=cdc_tr_main, 
                    method="lm", 
...
# Train test split
cdc_index_top20 <- createDataPartition(cdc$HighBP, p = 0.8, list = FALSE)
...
# Fit the model
model_top20 <- train(HighBP ~ . -county -state -CHD -HighChol -Stroke -pop -cruParticipate -Obesity -Unemploy -PhysInactivity -Parks, 
                    data=cdc_tr_top20, 
                    method="lm", 
...
# Train test split
cdc_index_top15 <- createDataPartition(cdc$HighBP, p = 0.8, list = FALSE)
...
# Fit the model
model_top15 <- train(HighBP ~ . -county -state -CHD -HighChol -Stroke -pop -cruParticipate -Obesity -Unemploy -PhysInactivity -Parks -SNAPrecipients -Pharmacies -MedHouseIncome -CholMedElegible -UrbanRural,  
                    data=cdc_tr_top15, 
                    method="lm", 
...
# Train test split
cdc_index_top10 <- createDataPartition(cdc$HighBP, p = 0.8, list = FALSE)
...
# Fit the model
model_top10 <- train(HighBP ~ . -county -state -CHD -HighChol -Stroke -pop -cruParticipate -Obesity -Unemploy -PhysInactivity -Parks -SNAPrecipients -Pharmacies -MedHouseIncome -CholMedElegible -UrbanRural -Hospitals -MedHomeValue -pcp -CardioPhys -Broadband,  
                    data=cdc_tr_top10, 
                    method="lm", 
...
# Train test split
cdc_index_top5 <- createDataPartition(cdc$HighBP, p = 0.8, list = FALSE)
...
# Fit the model
model_top5 <- train(HighBP ~ . -county -state -CHD -HighChol -Stroke -pop -cruParticipate -Obesity -Unemploy -PhysInactivity -Parks -SNAPrecipients -Pharmacies -MedHouseIncome -CholMedElegible -UrbanRural -Hospitals -MedHomeValue -pcp -CardioPhys -Broadband -AirQuality -HealthIns -Age65Plus -Smoker -Poverty,  
                    data=cdc_tr_top5, 
                    method="lm", 
...
# Train test split
cdc_index_cycle <- createDataPartition(cdc$HighBP, p = 0.8, list = FALSE)
...
for (y_var in y_var_list){
  # Fit the model
  model_cycle <- train(reformulate(y_var, "HighBP"),
                       data=cdc_tr_cycle, 
                       method="lm", 
...
# Train test split
cdc_index_multiX <- createDataPartition(cdc$HighBP, p = 0.8, list = FALSE)
...
for (y_var in y_var_list_multiX){
  # Fit the model
  model_multiX <- train(reformulate(y_var, "HighBP"),
                       data=cdc_tr_multiX, 
                       method="lm", 
...
HighBP_Coef <- pivot_wider(df, names_from = model, values_from = Coefficients)

# Round the coefficients to 4 decimal places
HighBP_Coef[-1] <- round(HighBP_Coef[-1], 3)
...
HighBP_Coef$Variable <- paste(HighBP_Coef$Variable, "_HighBP", sep = "")
...
## Top5 Model with Interactions .82 goes to .827 UrbanRUral EdLessColl and CholScreen significant
### Poverty .86 bpmUse EdLessColl CholMedNonAdhear Diabetes significant
### Full model less other targets and include top 5 features from variable importance shown in (main)
```{r}
# Set the seed
set.seed(2112)
...
# Fit the model
model_top5 <- train(HighBP ~ . -county -state -CHD -HighChol -Stroke -pop -cruParticipate -Obesity -Unemploy -PhysInactivity -Parks -SNAPrecipients -Pharmacies -MedHouseIncome -CholMedElegible -Hospitals -MedHomeValue -pcp -CardioPhys -Broadband -AirQuality -HealthIns -Age65Plus -Smoker + UrbanRural*bpmUse + UrbanRural*EdLessColl + UrbanRural*CholMedNonAdhear + UrbanRural*CholScreen + UrbanRural*Diabetes + Poverty*bpmUse + Poverty*EdLessColl + Poverty*CholMedNonAdhear + Poverty*CholScreen + Poverty*Diabetes,  
                    data=cdc_tr_top5, 
                    method="lm", 
                    trControl = control_top5)
