...
date: "`r Sys.Date()`"
format: html
subtitle: Tertiary document for specific (HiChol) target models
---
...
# Train test split
cdc_index_full <- createDataPartition(cdc$HighChol, p = 0.8, list = FALSE)
...
# Fit the model
model_full <- train(HighChol ~ . -county -state, 
                    data=cdc_tr_full, 
                    method="lm", 
...
# Train test split
cdc_index_main <- createDataPartition(cdc$HighChol, p = 0.8, list = FALSE)
...
# Fit the model
model_main <- train(HighChol ~ . -county -state -CHD -HighBP -Stroke , 
                    data=cdc_tr_main, 
                    method="lm", 
...
# Train test split
cdc_index_top20 <- createDataPartition(cdc$HighChol, p = 0.8, list = FALSE)
...
# Fit the model
model_top20 <- train(HighChol ~ . -county -state -CHD -HighBP -Stroke -Obesity -UrbanRural -Parks -CardioPhys -MedHouseIncome -Hospitals, 
                    data=cdc_tr_top20, 
                    method="lm", 
...
# Train test split
cdc_index_top15 <- createDataPartition(cdc$HighChol, p = 0.8, list = FALSE)
...
# Fit the model
model_top15 <- train(HighChol ~ . -county -state -CHD -HighBP -Stroke -Obesity -UrbanRural -Parks -CardioPhys -MedHouseIncome -Hospitals -pop -Broadband -MedHomeValue -cruParticipate -Unemploy,  
                    data=cdc_tr_top15, 
                    method="lm", 
...
# Train test split
cdc_index_top10 <- createDataPartition(cdc$HighChol, p = 0.8, list = FALSE)
...
# Fit the model
model_top10 <- train(HighChol ~ . -county -state -CHD -HighBP -Stroke -Obesity -UrbanRural -Parks -CardioPhys -MedHouseIncome -Hospitals -pop -Broadband -MedHomeValue -cruParticipate -Unemploy -SNAPrecipients -Poverty -pcp -Diabetes -Pharmacies,  
                    data=cdc_tr_top10, 
                    method="lm", 
...
# Train test split
cdc_index_top5 <- createDataPartition(cdc$HighChol, p = 0.8, list = FALSE)
...
# Fit the model
model_top5 <- train(HighChol ~ . -county -state -CHD -HighBP -Stroke -Obesity -UrbanRural -Parks -CardioPhys -MedHouseIncome -Hospitals -pop -Broadband -MedHomeValue -cruParticipate -Unemploy -SNAPrecipients -Poverty -pcp -Diabetes -Pharmacies -PhysInactivity -CholMedNonAdhear -CholMedElegible -Smoker -AirQuality,  
                    data=cdc_tr_top5, 
                    method="lm", 
...
# Train test split
cdc_index_cycle <- createDataPartition(cdc$HighChol, p = 0.8, list = FALSE)
...
for (y_var in y_var_list){
  # Fit the model
  model_cycle <- train(reformulate(y_var, "HighChol"),
                       data=cdc_tr_cycle, 
                       method="lm", 
...
# Train test split
cdc_index_multiX <- createDataPartition(cdc$HighChol, p = 0.8, list = FALSE)
...
for (y_var in y_var_list_multiX){
  # Fit the model
  model_multiX <- train(reformulate(y_var, "HighChol"),
                       data=cdc_tr_multiX, 
                       method="lm", 
...
HighChol_Coef <- pivot_wider(df, names_from = model, values_from = Coefficients)

# Round the coefficients to 4 decimal places
HighChol_Coef[-1] <- round(HighChol_Coef[-1], 3)
...
HighChol_Coef$Variable <- paste(HighChol_Coef$Variable, "_HighChol", sep = "")
...
## Top5 Model with interactions UrbanRural and Poverty (R2 75% goes up to 78%)
### UrbanRural interactions w/EdLessColl HealthIns CholScreen significant
### POverty interactions w/EdLessColl bpmUse HealthIns CholScreen
### Full model less other targets and include top 5 features from variable importance shown in (main)
```{r}
# Set the seed
set.seed(2112)
...
# Fit the model
model_top5 <- train(HighChol ~ . -county -state -CHD -HighBP -Stroke -Parks -CardioPhys -MedHouseIncome -Hospitals -pop -Broadband -MedHomeValue -cruParticipate -Unemploy -SNAPrecipients -pcp -Diabetes -Pharmacies -PhysInactivity -CholMedNonAdhear -CholMedElegible -Smoker -AirQuality + UrbanRural*EdLessColl + UrbanRural*bpmUse + UrbanRural*HealthIns + UrbanRural*CholScreen + UrbanRural*Age65Plus + Poverty*EdLessColl + Poverty*bpmUse + Poverty*HealthIns + Poverty*CholScreen + Poverty*Age65Plus,  
                    data=cdc_tr_top5, 
                    method="lm", 
                    trControl = control_top5)
