...
cdc_HighChol_full <- cdc %>% select(-c(county, state,))

cdc_HighChol_full$UrbanRural <- factor(ifelse(cdc_HighChol_full$UrbanRural %in% c("Large_Urban", "LargeFringe_Urban", "MediumSmall_Urban"), 1, 0))

#train test split
cdc_index_HighChol_full <- createDataPartition(cdc_HighChol_full$CHD, p = 0.8, list = FALSE)
cdc_tr_HighChol_full <- cdc_HighChol_full[cdc_index_HighChol_full, ]
cdc_te_HighChol_full <- cdc_HighChol_full[-cdc_index_HighChol_full, ]

#Rsquared 95%
control_HighChol_full <- trainControl(method="repeatedcv", number =5, repeats=3)
model_HighChol_full <- train(CHD ~ ., data=cdc_tr_HighChol_full, method="lm", trControl = control_HighChol_full)
print(model_HighChol_full$resample)
model_HighChol_full

importance_HighChol_full <- varImp(model_HighChol_full, scale=TRUE)
plot(importance_HighChol_full)
...
#train test split
cdc_index__full_Stroke <- createDataPartition(cdc__full_Stroke$HighChol, p = 0.8, list = FALSE)
...
model__full_Stroke <- train(HighChol ~ ., data=cdc_tr__full_Stroke, method="lm", trControl = control__full_Stroke)
