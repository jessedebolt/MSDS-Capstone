...
## Validate the Predictions on test data of Model top5
```{r}
...
## top 5 model as glm?? not using full model but kkeeping code for deviance and aic, maybe
```{r}
# Fit the model
...
cdc$CHD_logit <- log(cdc$CHD/100 + 1e-8) 

# Train test split
cdc_index_top5 <- createDataPartition(cdc$CHD_logit, p = 0.8, list = FALSE)
cdc_tr_top5 <- cdc[cdc_index_top5, ]
cdc_te_top5 <- cdc[-cdc_index_top5, ]
...
# Fit the model
model_top5 <- train(CHD_logit ~ . -CHD -Stroke -HighChol -HighBP -bpmUse -Unemploy
...
# Make predictions on the test set
predictions_logit <- predict(model_top5, newdata = cdc_te_top5)

# Transform predictions back to the original scale
predictions <- 100 / (1 + exp(-predictions_logit))
...
## Validate the Predictions on test data of Model top5 (results seem worse than just lm w/o logit)
```{r}
# Calculate the mean squared error of the predictions
mse <- mean((cdc_te_top5$CHD - predictions)^2)
print(paste("Mean Squared Error:", mse))
...
## testing model with interaction, gets 98% R2 oops that was with CHD logit. w/o it's actually 90%
```{r}
# Remove 'county', 'state', and other unnecessary columns before splitting 
cdc <- cdc[, !(names(cdc) %in% c('county', 'state', 'Stroke', 'HighChol', 'HighBP', 'bpmUse', 'Unemploy', 'MedHomeValue', 'CholMedNonAdhear', 'PhysInactivity', 'cruParticipate', 'AirQuality', 'Pharmacies', 'CardioPhys', 'Hospitals', 'Obesity', 'pop', 'Parks', 'MedHouseIncome', 'Poverty', 'pcp', 'CholMedElegible', 'UrbanRural', 'EdLessColl', 'CholScreen'))]

# Create the interaction term
cdc$Smoker_Diabetes <- cdc$Smoker * cdc$Diabetes
