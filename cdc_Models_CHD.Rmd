...
# Train test split
cdc_index_main <- createDataPartition(cdc$CHD, p = 0.8, list = FALSE)
...
# Fit the model
model_main <- train(CHD ~ . -county -state -Stroke -HighChol -HighBP -bpmUse, 
                    data=cdc_tr_main, 
                    method="lm", 
...
# Fit the model
model_top20 <- train(CHD ~ . -county -state -Stroke -HighChol -HighBP -bpmUse -Unemploy -MedHomeValue -Diabetes -CholMedNonAdhear -PhysInactivity, 
                    data=cdc_tr_top20, 
                    method="lm", 
...
# Fit the model
model_top15 <- train(CHD ~ . -county -state -Stroke -HighChol -HighBP -bpmUse -Unemploy -MedHomeValue -Diabetes -CholMedNonAdhear -PhysInactivity -cruParticipate -AirQuality -Pharmacies -CardioPhys -Hospitals,  
                    data=cdc_tr_top15, 
                    method="lm", 
...
# Fit the model
model_top10 <- train(CHD ~ . -county -state -Stroke -HighChol -HighBP -bpmUse -Unemploy -MedHomeValue -Diabetes -CholMedNonAdhear -PhysInactivity -cruParticipate -AirQuality -Pharmacies -CardioPhys -Hospitals -Obesity -pop -Parks -MedHouseIncome -Poverty,
                    data=cdc_tr_top10, 
                    method="lm", 
...
```{r}
# Fit the model
model_glm_poisson <- glm(CHD ~ . -Stroke -HighChol -HighBP -bpmUse -Unemploy -MedHomeValue -Diabetes -CholMedNonAdhear -PhysInactivity -cruParticipate -AirQuality -Pharmacies -CardioPhys -Hospitals -Obesity -pop -Parks -MedHouseIncome -Poverty -pcp -CholMedElegible -UrbanRural -EdLessColl -CholScreen, 
...
## Log CHD and top 5 model R2 89.4%
```{r}
# Libraries
library(caret)
library(ggplot2)
...
# Fit the model
model_top5_logit <- train(CHD_logit ~ . -CHD -Stroke -HighChol -HighBP -bpmUse -Unemploy
                    -MedHomeValue -Diabetes -CholMedNonAdhear -PhysInactivity
                    -cruParticipate -AirQuality -Pharmacies -CardioPhys
                    -Hospitals -Obesity -pop -Parks -MedHouseIncome -Poverty
                    -pcp -CholMedElegible -UrbanRural -EdLessColl -CholScreen, 
...
## testing model with interaction, gets 90% R2 (no better with Smoker*Diabetes)
```{r}
# Remove 'county', 'state', and other unnecessary columns before splitting 
cdc_SD <- cdc[, !(names(cdc) %in% c('county', 'state', 'Stroke', 'HighChol', 'HighBP', 'bpmUse', 'Unemploy', 'MedHomeValue', 'CholMedNonAdhear', 'PhysInactivity', 'cruParticipate', 'AirQuality', 'Pharmacies', 'CardioPhys', 'Hospitals', 'Obesity', 'pop', 'Parks', 'MedHouseIncome', 'Poverty', 'pcp', 'CholMedElegible', 'UrbanRural', 'EdLessColl', 'CholScreen'))]
