---
title: "MSDS 2023 Capstone Project"
author: "Jesse DeBolt & Isaac Johnson"
date: "`r Sys.Date()`"
format: html
subtitle: Tertiary document for specific (CHD) target models
---

#.
# Setup
#.

### Load libraries and knit setup
```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE, message = FALSE, warning = FALSE)

library(readr)
library(caret)
library(dplyr)
library(tidyverse)


```


### Read in dataset and descriptions
```{r}
cdc <- read_csv("data/CDC_all.csv")
TableDesc <- read_csv("data_support/Table_Descriptions.csv")

```

#.
# Data Cleaning
#.

## Feature reduction
### Remove unneeded/duplicated columns
```{r}
# Select and rearrange columns
cdc <- cdc %>% select(c("county", "state", "UrbanRural", "CHD", "HighBP", "HighChol", "Stroke", "Diabetes", "Obesity", "PhysInactivity", "Smoker", "Age65Plus","CholScreen", "CholMedNonAdhear", "CholMedElegible", "cruParticipate", "bpmUse", "MedHouseIncome", "Poverty", "Unemploy", "SNAPrecipients", "EdLessColl", "MedHomeValue", "HealthIns", "AirQuality", "Parks", "Broadband","pcp", "CardioPhys", "Hospitals", "Pharmacies", "pop"))

```


## Modify features
### Change Urban/Rural to binary
```{r}
# Convert to 1=Urban, 0=Rural as factor
cdc$UrbanRural <- factor(ifelse(cdc$UrbanRural %in% c("Large_Urban", "LargeFringe_Urban", "MediumSmall_Urban"), 1, 0))

```

#.
# CHD Linear Models
#.

## Full Model
### Full model with all features (full)
```{r}
# Set the seed
set.seed(2112)

# Train test split
cdc_index_full <- createDataPartition(cdc$CHD, p = 0.8, list = FALSE)
cdc_tr_full <- cdc[cdc_index_full, ]
cdc_te_full <- cdc[-cdc_index_full, ]

# Set control requirements
control_full <- trainControl(method="repeatedcv", 
                             number =5,
                             repeats=3)

# Fit the model
model_full <- train(CHD ~ . -county -state, 
                    data=cdc_tr_full, 
                    method="lm", 
                    trControl = control_full)

print(model_full$resample)
model_full

importance_full <- varImp(model_full, scale=TRUE)
plot(importance_full)

```


## Top20 Model
### Full model less other targets and include top 20 features from variable importance shown in (main)
```{r}
# Set the seed
set.seed(2112)

# Train test split
cdc_index_top20 <- createDataPartition(cdc$CHD, p = 0.8, list = FALSE)
cdc_tr_top20 <- cdc[cdc_index_top20, ]
cdc_te_top20 <- cdc[-cdc_index_top20, ]

# Set control requirements
control_top20 <- trainControl(method="repeatedcv", 
                             number =5,
                             repeats=3)

# Fit the model
model_top20 <- train(CHD ~ . -county -state -MedHomeValue -CardioPhys -PhysInactivity -Pharmacies -Unemploy -Hospitals -EdLessColl -Broadband -pop, 
                    data=cdc_tr_top20, 
                    method="lm", 
                    trControl = control_top20)

print(model_top20$resample)
model_top20

importance_top20 <- varImp(model_top20, scale=TRUE)
plot(importance_top20)

```


## Top15 Model
### Full model less other targets and include top 15 features from variable importance shown in (main)
```{r}
# Set the seed
set.seed(2112)

# Train test split
cdc_index_top15 <- createDataPartition(cdc$CHD, p = 0.8, list = FALSE)
cdc_tr_top15 <- cdc[cdc_index_top15, ]
cdc_te_top15 <- cdc[-cdc_index_top15, ]

# Set control requirements
control_top15 <- trainControl(method="repeatedcv", 
                             number =5,
                             repeats=3)

# Fit the model
model_top15 <- train(CHD ~ . -county -state -MedHomeValue -CardioPhys -PhysInactivity -Pharmacies -Unemploy -Hospitals -EdLessColl -Broadband -pop -Obesity -UrbanRural -SNAPrecipients -Poverty -AirQuality,  
                    data=cdc_tr_top15, 
                    method="lm", 
                    trControl = control_top15)

print(model_top15$resample)
model_top15

importance_top15 <- varImp(model_top15, scale=TRUE)
plot(importance_top15)

```


## Top10 Model
### Full model less other targets and include top 10 features from variable importance shown in (main)
```{r}
# Set the seed
set.seed(2112)

# Train test split
cdc_index_top10 <- createDataPartition(cdc$CHD, p = 0.8, list = FALSE)
cdc_tr_top10 <- cdc[cdc_index_top10, ]
cdc_te_top10 <- cdc[-cdc_index_top10, ]

# Set control requirements
control_top10 <- trainControl(method="repeatedcv", 
                             number =5,
                             repeats=3)

# Fit the model
model_top10 <- train(CHD ~ . -county -state -MedHomeValue -CardioPhys -PhysInactivity -Pharmacies -Unemploy -Hospitals -EdLessColl -Broadband -pop -Obesity -UrbanRural -SNAPrecipients -Poverty -AirQuality -CholScreen -Parks -Diabetes -cruParticipate -MedHouseIncome,
                    data=cdc_tr_top10, 
                    method="lm", 
                    trControl = control_top10)


print(model_top10$resample)
model_top10

importance_top10 <- varImp(model_top10, scale=TRUE)
plot(importance_top10)

```


## Top5 Model
### Full model less other targets and include top 5 features from variable importance shown in (main)
```{r}
# Remove 'county' and 'state' columns before splitting (otherwise caused issues with predictions)
cdc <- cdc[, !(names(cdc) %in% c('county', 'state'))]

# Set the seed
set.seed(2112)

# Train test split
cdc_index_top5 <- createDataPartition(cdc$CHD, p = 0.8, list = FALSE)
cdc_tr_top5 <- cdc[cdc_index_top5, ]
cdc_te_top5 <- cdc[-cdc_index_top5, ]

# Set control requirements
control_top5 <- trainControl(method="repeatedcv", 
                             number =5,
                             repeats=3)

# Fit the model
model_top5 <- train(CHD ~ . -MedHomeValue -CardioPhys -PhysInactivity -Pharmacies -Unemploy -Hospitals -EdLessColl -Broadband -pop -Obesity -UrbanRural -SNAPrecipients -Poverty -AirQuality -CholScreen -Parks -Diabetes -cruParticipate -MedHouseIncome -bpmUse -pcp -CholMedNonAdhear -CholMedElegible -HealthIns, 
                    data=cdc_tr_top5, 
                    method="lm", 
                    trControl = control_top5)

print(model_top5$resample)
model_top5

importance_top5 <- varImp(model_top5, scale=TRUE)
plot(importance_top5)

```


### Obtaining the r squared results from each model
```{r}
# Access R-squared
full_rsquared <- (summary(model_full)$r.squared)
#main_rsquared <- (summary(model_main)$r.squared)
top20_rsquared <- (summary(model_top20)$r.squared)
top15_rsquared <- (summary(model_top15)$r.squared)
top10_rsquared <- (summary(model_top10)$r.squared)
top5_rsquared <- (summary(model_top5)$r.squared)

# Print R-squared
print(paste("Full model R-squared =", sprintf("%.5f", full_rsquared)))
#print(paste("Main model R-squared =", sprintf("%.5f", main_rsquared)))
print(paste("Top20 model R-squared =", sprintf("%.5f", top20_rsquared)))
print(paste("Top15 model R-squared =", sprintf("%.5f", top15_rsquared)))
print(paste("Top10 model R-squared =", sprintf("%.5f", top10_rsquared)))
print(paste("Top5 model R-squared =", sprintf("%.5f", top5_rsquared)))

```


## Cycle Model
### Model with one variable at a time (cycle)
```{r}
# Set the seed
set.seed(2112)

# Train test split
cdc_index_cycle <- createDataPartition(cdc$CHD, p = 0.8, list = FALSE)
cdc_tr_cycle <- cdc[cdc_index_cycle, ]
cdc_te_cycle <- cdc[-cdc_index_cycle, ]

# Set control requirements
control_cycle <- trainControl(method="repeatedcv", 
                             number =5,
                             repeats=3)

# List of y variables
y_var_list <- c("UrbanRural", "Diabetes", "Obesity",
                "PhysInactivity", "Smoker", "Age65Plus",
                "CholScreen", "CholMedNonAdhear",
                "CholMedElegible", "cruParticipate",
                "MedHouseIncome", "Poverty",
                "Unemploy", "SNAPrecipients", "EdLessColl",
                "MedHomeValue", "HealthIns", "AirQuality",
                "Parks", "Broadband", "pcp", "CardioPhys",
                "Hospitals", "Pharmacies", "pop")

# Initialize an empty data frame to store results
cycle_results <- data.frame()

# Loop through each y variable
for (y_var in y_var_list){
  # Fit the model
  model_cycle <- train(reformulate(y_var, "CHD"),
                       data=cdc_tr_cycle, 
                       method="lm", 
                       trControl = control_cycle)
  
  # Extract metrics
  rmse <- model_cycle$results$RMSE[which.min(model_cycle$results$RMSE)]  # Best RMSE
  r_squared <- max(model_cycle$results$Rsquared)  # Best R-squared

  # Add results to data frame
  cycle_results <- rbind(cycle_results, data.frame(y_var = y_var, RMSE = rmse, R_Squared = r_squared))
}
  
# Sort the results by R_Squared in descending order
sorted_cycle_results <- cycle_results[order(-cycle_results$R_Squared),]

# Print the sorted results
print(sorted_cycle_results)

```


#.
# CHD Linear Model with interactions
#.

## Add previous grouped models to the results table
```{r}
# Put models in a list
model_list <- list(model_top5 = model_top5, 
                   model_top10 = model_top10, 
                   model_top15 = model_top15, 
                   model_top20 = model_top20, 
                   model_full = model_full)

# Loop through each model
for (model_name in names(model_list)) {
  # Extract model from list
  model <- model_list[[model_name]]
  
  # Extract metrics
  rmse <- model$results$RMSE[which.min(model$results$RMSE)]  # Best RMSE
  r_squared <- max(model$results$Rsquared)  # Best R-squared

  # Add results to data frame
  cycle_results <- rbind(cycle_results, data.frame(y_var = model_name, RMSE = rmse, R_Squared = r_squared))
}

# Sort the results by R_Squared in descending order
sorted_cycle_results <- cycle_results[order(-cycle_results$R_Squared),]

print(sorted_cycle_results)

```


## Interactions between Urban/Rural and various factors
```{r}
# Set the seed
set.seed(2112)

# Train test split
cdc_index_multiX <- createDataPartition(cdc$CHD, p = 0.8, list = FALSE)
cdc_tr_multiX <- cdc[cdc_index_multiX, ]
cdc_te_multiX <- cdc[-cdc_index_multiX, ]

# Set control requirements
control_multiX <- trainControl(method="repeatedcv", 
                             number =5,
                             repeats=3)

# List of y variables
y_var_list_multiX <- c("UrbanRural*Smoker",
                "UrbanRural*Diabetes",
                "UrbanRural*Obesity",
                "UrbanRural*PhysInactivity",
                "UrbanRural*Age65Plus",
                "UrbanRural*CholScreen",
                "UrbanRural*CholMedNonAdhear",
                "UrbanRural*CholMedElegible",
                "UrbanRural*cruParticipate",
                "UrbanRural*MedHouseIncome",
                "UrbanRural*Poverty",
                "UrbanRural*Unemploy",
                "UrbanRural*SNAPrecipients",
                "UrbanRural*MedHomeValue",
                "UrbanRural*HealthIns",
                "UrbanRural*AirQuality",
                "UrbanRural*Parks",
                "UrbanRural*Broadband",
                "UrbanRural*pcp",
                "UrbanRural*CardioPhys",
                "UrbanRural*Hospitals",
                "UrbanRural*Pharmacies",
                "UrbanRural*pop"
                )

# Initialize an empty data frame to store results
multiX_results <- data.frame()

# Loop through each y variable
for (y_var in y_var_list_multiX){
  # Fit the model
  model_multiX <- train(reformulate(y_var, "CHD"),
                       data=cdc_tr_multiX, 
                       method="lm", 
                       trControl = control_multiX)
  
  # Extract metrics
  rmse <- model_multiX$results$RMSE[which.min(model_multiX$results$RMSE)]  # Best RMSE
  r_squared <- max(model_multiX$results$Rsquared)  # Best R-squared

  # Add results to data frame
  multiX_results <- rbind(multiX_results, data.frame(y_var = y_var, RMSE = rmse, R_Squared = r_squared))
}
  
# Sort the results by R_Squared in descending order
sorted_multiX_results <- multiX_results[order(-multiX_results$R_Squared),]

# Print the sorted results
print(sorted_multiX_results)

```

#.
# Results
#.

## Combined results RMSE and R^2
```{r}
# Add lists together into new table
all_results <- rbind(sorted_cycle_results, sorted_multiX_results)

# Sort results by RMSE values
all_results <- all_results[order(-all_results$R_Squared),]

print(all_results)
```


## Combined results: Coefficients
```{r}
# List of models
models <- list(full = model_full, 
               #main = model_main, 
               top20 = model_top20, 
               top15 = model_top15,
               top10 = model_top10,
               top5 = model_top5)

# Create an empty list to store coefficients
coeffs_list <- list()

# Loop over each model
for (model_name in names(models)) {
  # Extract the coefficients of the model
  coeffs <- coef(models[[model_name]]$finalModel)
  
  # Convert to data frame and set column names
  coeffs_df <- data.frame(Variable = names(coeffs), Coefficients = coeffs, stringsAsFactors = FALSE)
  
  # Add a column with the name of the model
  coeffs_df$model <- model_name
  
  # Add the data frame to the list
  coeffs_list[[model_name]] <- coeffs_df
}

# Bind all data frames in the list into a single data frame
df <- bind_rows(coeffs_list)

# Convert to wide format
CHD_Coef <- pivot_wider(df, names_from = model, values_from = Coefficients)

# Round the coefficients to 4 decimal places
CHD_Coef[-1] <- round(CHD_Coef[-1], 3)

# View the data frame
print(CHD_Coef)

```


### Add label to each variable for this model set
```{r}
# Add "_HighBP" to each variable name
CHD_Coef$Variable <- paste(CHD_Coef$Variable, "_CHD", sep = "")

# View the data frame
print(CHD_Coef)

```


### Combine all four (CHD, HighBP, HighChol, Stroke) coefficient tables
NOTE: run other cdc_Models_xyz files first, keep results in global environment, then run this final version, which pulls from all others.
```{r}
# # Stack the dataframes
# All_Coef <- bind_rows(CHD_Coef, HighBP_Coef, HighChol_Coef, Stroke_Coef)
# 
# # Sort the dataframe by the 'Variable' column
# All_Coef <- All_Coef %>% arrange(Variable)
# 
# # View the stacked and sorted dataframe
# print(All_Coef)

```

#.
# Predictions
#.

# Predictions on test data of Model top5
```{r}
# Make predictions on the test set
predictions <- predict(model_top5, newdata = cdc_te_top5)

# Look at the first few predictions
head(predictions)

```


## Validate the Predictions on test data of Model top5 (88% R2 on test data)
```{r}
# Calculate the mean squared error of the predictions
mse <- mean((cdc_te_top5$CHD - predictions)^2)
print(paste("Mean Squared Error:", mse))

# Calculate the root mean squared error of the predictions
rmse <- sqrt(mse)
print(paste("Root Mean Squared Error:", rmse))

#Easier way to get RMSE
RMSE(predictions, cdc_te_top5$CHD)

#R2
R2(predictions, cdc_te_top5$CHD)

#RMSE R2 MAE summary
postResample(pred = predictions, obs = cdc_te_top5$CHD)

#The prediction error RMSE is .55, representing an error rate of .55/mean(cdc_te_top5$CHD) = 6.6%, which is good.
.55/mean(cdc_te_top5$CHD)
```



## top 5 model as glm for measurement use deviance and aic, maybe
```{r}

# Set the seed
set.seed(2112)

# Train test split
cdc_index_top5 <- createDataPartition(cdc$CHD, p = 0.8, list = FALSE)
cdc_tr_top5 <- cdc[cdc_index_top5, ]
cdc_te_top5 <- cdc[-cdc_index_top5, ]

# Set control requirements
control_top5 <- trainControl(method="repeatedcv", 
                             number =5,
                             repeats=3)

# Fit the model
model_glm_poisson <- glm(CHD ~ . -MedHomeValue -CardioPhys -PhysInactivity -Pharmacies -Unemploy -Hospitals -EdLessColl -Broadband -pop -Obesity -UrbanRural -SNAPrecipients -Poverty -AirQuality -CholScreen -Parks -Diabetes -cruParticipate -MedHouseIncome -bpmUse -pcp -CholMedNonAdhear -CholMedElegible -HealthIns, 
                         family = poisson, data = cdc_tr_top5)

print(model_glm_poisson$resample)
model_glm_poisson

importance_top5 <- varImp(model_glm_poisson, scale=TRUE)
plot(importance_top5)


# Get summary
summary(model_glm_poisson)

# Get deviance
deviance_glm <- deviance(model_glm_poisson)
print(paste("Deviance:", deviance_glm))

# Get AIC
aic_glm <- AIC(model_glm_poisson)
print(paste("AIC:", aic_glm))

```


## Log CHD and top 5 model R2 98%
```{r}
# Libraries
library(caret)
library(ggplot2)

cdc_logit <- cdc

# Add a small constant to avoid taking log of zero
cdc_logit$CHD_logit <- log(cdc_logit$CHD/100 + 1e-8) 

# Train test split
cdc_index_top5_logit <- createDataPartition(cdc_logit$CHD_logit, p = 0.8, list = FALSE)
cdc_tr_top5_logit <- cdc_logit[cdc_index_top5_logit, ]
cdc_te_top5_logit <- cdc_logit[-cdc_index_top5_logit, ]

# Set control requirements
control_top5_logit <- trainControl(method="repeatedcv", 
                             number =5,
                             repeats=3)

# Fit the model
model_top5_logit <- train(CHD_logit ~ . -MedHomeValue -CardioPhys -PhysInactivity -Pharmacies -Unemploy -Hospitals -EdLessColl -Broadband -pop -Obesity -UrbanRural -SNAPrecipients -Poverty -AirQuality -CholScreen -Parks -Diabetes -cruParticipate -MedHouseIncome -bpmUse -pcp -CholMedNonAdhear -CholMedElegible -HealthIns, 
                    data=cdc_tr_top5_logit, 
                    method="lm", 
                    trControl = control_top5_logit)

# Print the model summary
print(model_top5_logit)

# Make predictions on the test set
predictions_logit <- predict(model_top5_logit, newdata = cdc_te_top5_logit)

# Transform predictions back to the original scale
predictionsL <- 100 / (1 + exp(-predictions_logit))

# Print the predictions
head(predictionsL)



```

## Validate the Predictions on test data of Model top5 Log (results seem worse? than just lm w/o logit)
```{r}
# Calculate the mean squared error of the predictions
mse <- mean((cdc_te_top5_logit$CHD - predictionsL)^2)
print(paste("Mean Squared Error:", mse))

# Calculate the root mean squared error of the predictions
rmse <- sqrt(mse)
print(paste("Root Mean Squared Error:", rmse))

```


## testing top 5 + Diabetes model with interaction, gets 94.7% R2 (no better with Smoker*Diabetes)
```{r}
# Remove 'county', 'state', and other unnecessary columns before splitting 
cdc_SD <- cdc[, !(names(cdc) %in% c('county', 'state', 'MedHomeValue', 'CardioPhys', 'PhysInactivity', 'Pharmacies', 'Unemploy', 'Hospitals', 'EdLessColl', 'Broadband', 'pop', 'Obesity', 'UrbanRural', 'SNAPrecipients', 'Poverty', 'AirQuality', 'CholScreen', 'Parks', 'cruParticipate', 'MedHouseIncome', 'bpmUse', 'pcp', 'CholMedNonAdhear', 'CholMedElegible', 'HealthIns'))]

# Create the interaction term
cdc_SD$Smoker_Diabetes <- cdc_SD$Smoker * cdc_SD$Diabetes

# Set the seed
set.seed(2112)

# Train test split
cdc_index_top5SD <- createDataPartition(cdc_SD$CHD, p = 0.8, list = FALSE)
cdc_tr_top5SD <- cdc_SD[cdc_index_top5SD, ]
cdc_te_top5SD <- cdc_SD[-cdc_index_top5SD, ]

# Set control requirements
control_top5SD <- trainControl(method="repeatedcv", 
                             number =5,
                             repeats=3)

# Fit the model
model_top5SD <- train(CHD ~ ., 
                    data=cdc_tr_top5SD, 
                    method="lm", 
                    trControl = control_top5SD)

print(model_top5SD$resample)
model_top5SD

importance_top5SD <- varImp(model_top5SD, scale=TRUE)
plot(importance_top5SD)

summary(model_top5SD)
```




























