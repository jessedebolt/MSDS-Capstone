...
# Create the subset
cdc_subset <- cdc %>%
  select(c(5:46))  #(5:46) All excluding state and rural/urban hot coded
...
```{r}
# for (col in c(5:46)) {  #(5:70) All numeric excluding state and rural/urban hot coded
...
excluded_cols <- c("fips", "county", "state", "UrbanRural", "UrbanRural_MediumSmall_Urban", "UrbanRural_Rural", "UrbanRural_Large_Urban", "UrbanRural_LargeFringe_Urban" )
included_cols <- setdiff(names(cdc), excluded_cols)

for (col in included_cols) {
  if (is.numeric(cdc[[col]])) {
    hist(cdc[[col]], main = col)
  }
}
...
```{r}
cor_matrix <- cdc %>%
  select(c(5:46)) %>% cor() #(5:46) All excluding state and rural/urban hot coded
cor_matrix
...
```{r}
cdc_lm <- cdc %>% select (5:46,) #(5:46) All numeric excluding hot coded
...
cdc_pca2 <- cdc %>%
  select(c(5:42)) #(5:42) All excluding state and rural/urban hot coded
...
#### View the loadings of the first five priciple components which make up 66.77%
```{r}
pr_pca2$rotation[, 1:5]

```


#### Sort the loadings of the first priciple component
```{r}
loadings <- pr_pca2$rotation

sorted_PC1 <- sort(loadings[,"PC1"], decreasing = TRUE)
print(sorted_PC1)

```


#### Sort the loadings of the second priciple component
```{r}
loadings <- pr_pca2$rotation

sorted_PC1 <- sort(loadings[,"PC2"], decreasing = TRUE)
print(sorted_PC1)

```


#### Sort the loadings of the third priciple component
```{r}
loadings <- pr_pca2$rotation

sorted_PC1 <- sort(loadings[,"PC3"], decreasing = TRUE)
print(sorted_PC1)

```


#### Sort the loadings of the fourth priciple component
```{r}
loadings <- pr_pca2$rotation

sorted_PC1 <- sort(loadings[,"PC4"], decreasing = TRUE)
print(sorted_PC1)

```


#### Sort the loadings of the fifth priciple component
```{r}
loadings <- pr_pca2$rotation

sorted_PC1 <- sort(loadings[,"PC5"], decreasing = TRUE)
print(sorted_PC1)
...
```{r}
cdc_test <- cdc %>% select (3, 5:46, ) #(3, 5:46, ) state, all numeric excl.hot code
...
cdc_test <- cdc %>% select (5:46, ) #(5:46) All numeric excluding hot coded
...
### Policy intervention to increases the number of parks by 100% in rural areas
```{r}
...
### Policy intervention to increase/decrease various variables using linear regression models
...
#### Split the data into training, testing, and holdout sets. We'll use 70% of the data for training, 15% for testing, and 15% for holdout
```{r}
...
### Compare the model's performance before and after decreasing the 'Broadband' variable by 50%, we first need to create a new dataset with the increased 'Broadband' values
```{r}
cdc_increased <- cdc2
cdc_increased$Broadband <- cdc_increased$Broadband * 0.5

```


#### Split this new dataset into training, testing, and holdout sets, and create a new model
```{r}
...
### Compare the performance of the two models using your preferred metrics (e.g., RMSE, R-squared, etc.). For example:
...
```{r}
# Jesse commented out due to length to run and limited output
...
###Another way to see some correlations sorted
```{r}
...
```
### See highest correlations sorted desc (Removed duplicate correlations)
```{r}
...
### CHD scatterplots for most variables
```{r}
...
# Convert UrbanRural to a factor
cdc2$UrbanRural <- factor(cdc2$UrbanRural, labels = c("Rural", "Urban"))
...
### Multiple models (m4 is a good model! though not split train test sets)
```{r}
...
#94% Rsquared
m4 <- lm(CHD ~ Stroke + Age65Plus + HighChol + Smoker + HighBP + EdLessColl + MedHouseIncome + EdLessColl*MedHouseIncome,  data=cdc)
...
### Rpart model (fit) and Random Forest model (fit_rf)
```{r}
...
#from 4026 just trying another model, can probably delete
```{r}
#this gets rmse .347
#cdc2 <- cdc %>% select(5:70)
...
#cdc2 <- cdc %>% select(4:13,19,23:29,32,35,42)
#this is same as select(4:13,19,23:29,32,35,42) 
cdc2 <- cdc %>% dplyr::select("UrbanRural", "CHD", "HighBP", "HighChol", "Stroke", "Diabetes", "Obesity", "PhysInactivity", "Smoker", "Age65Plus", "MedHouseIncome", "EdLessColl", "MedHomeValue", "HealthIns", "AirQuality", "Parks", "Broadband", "pcp", "CardioPhys", "Hospitals", "pop")
...
plot(importance)
```
### Full model without states
```{r}
#create dataset for model
cdc_step <- cdc %>% dplyr::select(4:42)
...
### setting up model for Stepwise Feature Selection
```{r}
#library(MASS)

# Create the full model
# full.model <- lm(CHD ~ ., data = cdc_step)
```
...
### creating lists & datasets of variable names
```{r}
#create list of variable names ALL without states
cdc_names_wostates <- cdc %>% dplyr::select(4:46) %>% names()

#create data set ALL w/o states
cdc_wostates <- cdc %>% dplyr::select(4:46) #%>% names()

cdc_wostates$UrbanRural <- factor(ifelse(cdc_wostates$UrbanRural %in% c("Large_Urban", "LargeFringe_Urban", "MediumSmall_Urban"), 1, 0))
```

###Model with cdcAll w/o states
```{r}
#95% Rsquared
set.seed(123)
cdcAll_index <- createDataPartition(cdc_wostates$CHD, p = 0.8, list = FALSE)
cdcAll_tr <- cdc_wostates[cdcAll_index, ]
cdcAll_te <- cdc_wostates[-cdcAll_index, ]
control <- trainControl(method="repeatedcv", number =5, repeats=3)
mAll <- train(CHD ~ ., data=cdcAll_tr, method="lm", trControl = control)
print(mAll$resample)
mAll

importanceAll <- varImp(mAll, scale=TRUE)
plot(importanceAll)
```
...
# ↓↓↓↓↓ NEW(Isaac) 7/20 ↓↓↓↓↓
#.
Look at up above full model without states for full model chd ~ .
then these:
...
#create dataset for model (4:70)
cdc_basic <- cdc %>% dplyr::select("CHD", "Age65Plus", "Smoker", "HealthIns", "Parks", "Cardio100k", "Obesity", "MedHouseIncome", "Diabetes", "CholScreen", "UrbanRural", "SNAPrecipients", "Broadband", "AirQuality", "Pharmacies", "Hosp100k", "Pharm100k", "EdLessColl", "PhysInactivity", "Hospitals", "Poverty", "TotalCardio", "pcp", "TotalPCP")
...
###Full Model 5 now predicting High BP  87% R2
```{r}
set.seed(123)

#create dataset for model (5:46)
cdc_basic_bp <- cdc %>% dplyr::select("CHD", "HighBP", "HighChol", "Stroke", "Diabetes", "Obesity", "PhysInactivity", "Smoker", "Age65Plus", "CholScreen", "CholMedNonAdhear", "CholMedElegible", "cruParticipate", "bpmUse", "MedHouseIncome", "Poverty", "Unemploy", "SNAPrecipients", "EdLessColl", "MedHomeValue", "HealthIns", "AirQuality", "Parks", "Broadband", "pcp", "TotalPCP", "PSP1k", "CardioPhys", "TotalCardio", "Cardio100k", "Hospitals", "Hosp100k", "HospCR", "HospCIC", "HospED", "Pharmacies", "Pharm100k", "pop", "UrbanRural_Large_Urban", "UrbanRural_LargeFringe_Urban", "UrbanRural_MediumSmall_Urban", "UrbanRural_Rural") %>% dplyr::select(-c("CHD", "HighChol", "Stroke"))
...
#create dataset for model (4:42)
cdc_basic_bp <- cdc %>% dplyr::select("UrbanRural", "CHD", "HighBP", "HighChol", "Stroke", "Diabetes", "Obesity", "PhysInactivity", "Smoker", "Age65Plus", "CholScreen", "CholMedNonAdhear", "CholMedElegible", "cruParticipate", "bpmUse", "MedHouseIncome", "Poverty", "Unemploy", "SNAPrecipients", "EdLessColl", "MedHomeValue", "HealthIns", "AirQuality", "Parks", "Broadband", "pcp", "TotalPCP", "PSP1k", "CardioPhys", "TotalCardio", "Cardio100k", "Hospitals", "Hosp100k", "HospCR", "HospCIC", "HospED", "Pharmacies", "Pharm100k", "pop", "UrbanRural_Large_Urban", "UrbanRural_LargeFringe_Urban", "UrbanRural_MediumSmall_Urban", "UrbanRural_Rural") %>% dplyr::select(-c("CHD", "HighChol", "Stroke", "UrbanRural_Large_Urban", "UrbanRural_LargeFringe_Urban", "UrbanRural_MediumSmall_Urban", "UrbanRural_Rural"))
...
#create dataset for model (5:46)
cdc_basic_bp <- cdc %>% dplyr::select("HighBP", "Diabetes", "CholScreen", "bpmUse")
...
#create dataset for model (5:46)
cdc_basic_bp <- cdc %>% dplyr::select("HighBP", "Diabetes", "CholScreen", "bpmUse", "CholMedNonAdhear", "Smoker")
...
#create dataset for model (5:46)
cdc_basic_bp <- cdc %>% dplyr::select("HighBP", "Diabetes", "CholScreen", "bpmUse", "CholMedNonAdhear", "Poverty", "EdLessColl")
...
# 
# # Reduce initial number of features
# cdc_rfe2 <- cdc_step %>% dplyr::select("CHD", "HighBP", "HighChol", "Stroke", "Diabetes", "Obesity", "PhysInactivity", "Smoker", "Age65Plus", "bpmUse", "MedHouseIncome", "Poverty", "SNAPrecipients", "EdLessColl", "MedHomeValue", "HealthIns", "AirQuality", "Parks", "Broadband", "pcp", "TotalPCP", "PSP1k", "CardioPhys", "TotalCardio", "Cardio100k", "Hospitals", "Hosp100k", "HospED", "Pharmacies")
# 
# trainIndex_RFE2 <- createDataPartition(cdc_rfe2$CHD, p = .7, list = FALSE, times = 1)
# 
# cdcTrain_RFE2 <- cdc_rfe2[ trainIndex_RFE2,]
# cdcTest_RFE2  <- cdc_rfe2[-trainIndex_RFE2,]
# 
# x <- dplyr::select(cdcTrain_RFE2,-CHD)
# y <- cdcTrain_RFE2$CHD
# 
# ### Establish control features
# ctrl_RFE2 <- rfeControl(functions=rfFuncs, method="cv", number=10)
...
### Perform RFE model
```{r}
# result2 <- rfe(cdcTrain_RFE2[, -ncol(cdcTrain_RFE2)], cdcTrain_RFE2$CHD, sizes=c(1:ncol(cdcTrain_RFE2)-1), rfeControl=ctrl_RFE2)
# 
# print(result2)
# print(result2, type=c("g", "o"))
...
```{r}
# 
# print(result2$variables)
# 
# print(result2$optimum)
# 
# print(names(result2))
